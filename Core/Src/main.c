...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void lora_get_data(struct lora_dev *lora, u8 *buf, u32 size) {
	if (!strncmp((char *)buf, "tb4", strlen("tb4"))) {
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_Delay(2000);
	while (lora_init(&sx1278) != LORA_SUCCESS) {
		HAL_Delay(100);
	}
